"""Resolve and ensure a skill's declared dependencies.

Reads SKILL.md frontmatter for a `dependencies` block-list (or accepts
names as positional args), applies the resolution order from the
`dependency-resolution` rule, deploys the skill's bundled fallback if
nothing is found, and emits a path list under the conditional-load
instruction that lets the agent dedup per-file.
...
not present at any deployed location.
"""
...
def main(argv: list[str]) -> int:
    ...
    deps = argv if argv else _parse_frontmatter_dependencies(skill_root / "SKILL.md")
    if not deps:
        return 0
    ...
    for path in resolved_paths:
        print(f"- {path}")
    print()
    print("Load any listed dependencies not already in context.")
