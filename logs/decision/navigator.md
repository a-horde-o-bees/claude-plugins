...
Decisions governing the navigator system — project-structure indexing and MCP-exposed query tools.
...
**Explicit injection** — callers pass `project_dir` as an argument. Explicit, testable without env vars, matches the prior signature.

**Internal resolution** — navigator resolves project root via `plugin.get_project_dir()` itself. Callers just say "scan this subpath" or "load governance" with no path argument.
...
- **Constrains:** the scanner must walk absolute paths anchored on the resolved project root (not cwd-relative), so storing project-relative paths via `relpath` is mandatory to keep the paths table stable across working directories
