...
- raw `mcp` Python SDK / FastMCP 1.x / FastMCP 2.x / custom: FastMCP (standalone package, likely 2.x)
- version pin from pyproject.toml: not captured precisely
- import pattern observed: likely `from fastmcp import FastMCP`

### Python version floor
- `requires-python` value: not extracted from pyproject; `runtime.txt` pins 3.13.3

### Packaging
- build backend: not extracted
- lock file present: `uv.lock` implied (`uv sync`)
- version manager convention: `uv`

### Entry point
- `[project.scripts]` console script / `__main__.py` module / bare script / other: **bare script** (`main.py`) with CLI args parsed inside the script
- actual console-script name(s): none
- host-config snippet shape (uvx / uv run / pipx / python -m / absolute venv path): `uv run python main.py --transport stdio`

### Install workflow expected of end users
- install form + one-liner from README: `git clone ... && uv sync`

### Async and tool signatures
- sync `def` or `async def`: likely async given multi-transport support

### Type / schema strategy
- Pydantic / dataclasses / TypedDict / raw dict / Annotated: FastMCP-auto-derived (not captured directly)

### Testing
- pytest / pytest-asyncio / unittest / none: /tests/ directory present

### Dev ergonomics
- mcp dev / fastmcp dev / Inspector launcher / Makefile / Justfile / other: docker-compose for multi-transport dev
