...
50+ CLI flags and matching env vars; JSON config file via `--config`.
...
80+ structured tools with role-based permissions in categories:
...
### Programmatic embedding as first-class

`createConnection()` means this MCP server can run inside host processes as a library, not just as an external subprocess.
...
this MCP server can be a Node library inside another process, blurring server/client lines.
