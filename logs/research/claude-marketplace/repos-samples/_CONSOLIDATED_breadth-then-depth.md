...
No `monitors.json` in observed samples. Notifications, when produced, flow through the hook system (Stop, SubagentStop, Notification, TeammateIdle) directly. The samples surface this is a real gap: a plugin literally named "notifications" does not use the documented monitor channel — anyone searching for monitor examples would miss it. Either the surface is too new, the plugins predate it, or none have a polling-style use case. Equivalent functionality is delivered via hooks (PostToolUse + Stop + SubagentStop combinations) plus runtime MCP tool calls (e.g. `cdp_status`). Author awareness varies — at least one plugin has real-time diagnostic needs that a `monitors.json` could surface; the hook-based equivalent works but is not equivalent to a declarative monitor. Constrains the plugin's ability to do truly background work without piggy-backing on hook events. Appropriate to flag as a corpus-wide observation: monitors may be under-adopted relative to their advertised role.
