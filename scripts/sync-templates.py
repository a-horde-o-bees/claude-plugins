...
system's init() with force=True to sync templates to their deployed
locations in .claude/. Clears orphaned deployed files when templates
are removed.

Content systems are the template-based systems packages (rules,
conventions, patterns, log) — infrastructure systems like navigator
and permissions are excluded because their init has side effects
beyond template deployment.
...
CONTENT_SYSTEMS = ("rules", "conventions", "patterns", "log")


def _sync_plugin(plugin_dir: Path) -> list[str]:
    """Run each content system's init for one plugin. Returns paths touched."""
    synced: list[str] = []

    # Each plugin's sys.path is isolated so its `systems.*` and `plugin`
    # resolve to its own copies. Unload modules from any prior plugin first.
    for key in list(sys.modules):
        if (key == "plugin" or key.startswith("plugin.")
                or key == "systems" or key.startswith("systems.")):
            del sys.modules[key]
