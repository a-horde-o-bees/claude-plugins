...
description: Use when structuring a skill or any trigger-loaded entry-point doc into layers — metadata, entry point, deep content — so a false-positive trigger pays only for navigation and the workflow loads only on dispatch.
---
...
1. **Metadata** — frontmatter `name` + `description` (per /author-descriptions). Always in context. Sized to fire the trigger reliably across phrasings.
