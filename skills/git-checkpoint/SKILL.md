---
name: git-checkpoint
description: Run the current branch's full development checkpoint in one call — commit, push, and the project-configured lifecycle (direct, or PR → CI → merge → release), recursing into submodules. Use for "checkpoint", "ship it", or wrapping up a unit of work.
...
- `{auto}` — `--auto` present: hands-off. Threaded verbatim to `/git-pr-open` and `/git-pr-merge`; the leaves own what it bypasses (the description review gate; soft-blocker prompts; the pending-CI watch). Checkpoint adds no auto behavior of its own.
...
4. Routing pre-flight + land PR-governed submodules (before the parent commits) — one mechanical pass routes the parent and every submodule; any gap halts up front:
    ...
    2. {routes}: bash: `python3 ${CLAUDE_SKILL_DIR}/../git-pr-merge/scripts/pr.py routes` (append ` --paths {paths}` when {paths} is set) — one pass over the parent + every declared submodule, each repo's `{path, integration, sync, gaps, branch, perm, has_edits}` plus the combined gap list, all probes run in-process. The skill spends no context gathering routing
    ...
    5. For each {repo} in {routes}.repos with {repo}.path ≠ `.` (the submodules):
        ...
        3. If {repo}.integration is `direct`: {ledger} += `{sub}: direct (leaves push it)`; continue — left to the commit/push leaves' direct recursion
        4. {repo}.integration is `pr`:
            ...
            5. {sub-report}: skill: `/git-checkpoint --path pr` + ` --auto` if {auto} — full lifecycle in {sub}: recurse its own sub-submodules, commit → push → CI → open PR → merge (gated) → cleanup
...
6. Commit + push + CI (every context) — pass `--pin-only {p}` for each {p} in {pin-only} so the leaves skip submodules checkpoint already landed or pins:
    1. {commit-report}: skill: `/git-commit {paths}` + ` --auto` if {auto} + ` --pin-only {p}` for each {pin-only} — pass `--on-base` when {branch} == {default-branch} (an intentional base commit) so the leaf's base guard permits it. `--auto` auto-consumes the landed submodules' pin advances
    2. {push-report}: skill: `/git-push --branch {branch}` + ` --pin-only {p}` for each {pin-only}
