...
- **`skill:`** — the Skill tool: `skill: /<skill-name>`, written bare (no trailing parenthetical — the whole skill applies).
...
Reuse logic by factoring it into a `Call:` section or component file rather than repeating a `skill:`. The prefix is optional when the mechanism is clear, required when a flow mixes mechanisms.

Each action step sits on one side of a bridge between agent-facing instruction and mechanical code — write it as workflow, or delegate it to a script the workflow invokes:

- **Workflow** carries reasoning, judgment, and contextual sequencing the agent steers; orchestration of disparate systems (skill calls, tool invocations, agent spawns) whose composition depends on intermediate results; and the user-facing surface — review gates, clarifying questions, error-recovery dialogue.
- **Script** (a `bash:` call or invoked module) carries mechanically resolvable operations — parsing, slicing, filtering, aggregation, format conversion, deterministic classification: whatever a function with no agent context computes correctly.

Decide by asking whether a deterministic function with no agent context could produce the result: if yes, script; if it needs judgment, orchestration, or presentation, workflow. A script never demands a mid-run agent decision — that decision belongs in the workflow; a step never parses, filters, or transforms data inline — that work belongs in a script.
