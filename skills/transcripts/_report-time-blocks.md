...
- **Summary description** — the specific unified objective of its exchanges; broader than the per-exchange descriptions, written per /author-descriptions.
...
4. For each {session} in {sessions}, author any missing descriptions:
    ...
    3. For each {exchange} whose `description` is null:
        ...
        2. Compose a description per /author-descriptions
...
5. Coalesce **unblocked** exchanges into new blocks (skip any `session:exchange` in {blocked}):
    ...
    2. Compose a block summary per /author-descriptions.
