...
- **Invocations** — `callvars` (a `Call:` sees the caller's variables — same context, through a `[label](file.md)` component), `callskill` (`Call: /skill` dispatches via the Skill tool), `readnoexec` (`Read:` loads without executing), `apply` (`Apply … to:` runs a block through a skill as a lens), plus the `fidelity_call` / `fidelity_execute` / `fidelity_control` group below; the `[label](#anchor)` target form is exercised by `callbind` and the exit/return cases
...
`fidelity_call` and `fidelity_execute` are identical flows — a file-invocation buried among decoy steps, under the neutral prompt — differing only in the verb. The call's label looks self-sufficient ("append the line PASS"), so a weak verb could let the agent self-serve and never open the file; but `_record.md` overrides with a different action (`echo REC-Q8Z4`). The marker appears only if the verb pulled the agent into the file, so the hit-rate gap is the measurable evidence — a wording change is judged by whether it moves the rate, not by argument. `fidelity_control` is the group's negative control: it invokes the file with `Read:`, which loads without executing, so `REC-Q8Z4` must stay absent.
