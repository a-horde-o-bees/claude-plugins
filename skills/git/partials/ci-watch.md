...
- {ci-status} post-watch is one of `passed`, `failed`, `incomplete` (the `dispatched` and `no-runs` states from `verbs/ci.md` don't apply — by construction we always have a tracked in-flight set)
- `${CLAUDE_SKILL_DIR}/scripts/ci.py watch` blocks until each watched run completes, then re-lists all runs for the SHA and re-classifies via the same `classify_runs` logic `verbs/ci.md` uses
- Template emission matches the three matching states of `verbs/ci.md` § Report; emit verbatim, no inventing or paraphrasing
