...
- Submodule recursion is opt-out by default — each submodule has its own release cadence. The recursion lives in the other git verbs (`/git commit`, `/git push`, `/git ci`); cutting a release is a deliberate per-project act. Pass `--recurse-submodules` to opt in (each declared submodule then runs its own `/git release` flow before the parent records the new pin)
