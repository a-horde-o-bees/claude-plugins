...
5. Synthesize CHANGELOG + version:
    1. Spawn async agent to: Call: partials/release-synthesize.md (`{commit-range}`: `{commit-range}`, `{current-version}`: `{current-version}`, `{methodology}`: `{methodology}`)
...
8. Review gate:
    ...
    3. If `{decision}` is approve: go to step 9
