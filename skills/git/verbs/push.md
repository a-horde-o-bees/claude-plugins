...
- `verbs/doctor.md` — submodule conformance pre-check before any push.
- `verbs/commit.md` — auto-commit fallback for a dirty tree.
...
- `{pin-only}` — `--pin-only <path>` (repeatable); submodules the parent only pins, never direct-pushes. Set by `/git checkpoint` for PR-governed and vendored submodules — their changes land via their own PR or are read-only.

## Rules

- Pre-check submodule conformance via `/git doctor` at the top level; declines stop the flow
- Submodule recursion requires `.gitmodules` to declare `submodule.<name>.branch` — without that contract recursive push exits, never inferring it from the checked-out branch (a silent guess that could push wherever HEAD points). Checkpoint's routing pre-flight catches this up front; `/git doctor` writes the native key on approval
- A `{pin-only}` submodule (PR-governed or vendored) is never direct-pushed — a protected branch refuses direct pushes by design; its changes land via its own PR (`/git checkpoint` runs that lifecycle). The parent pins whatever sha it is on
...
2. Recurse into submodules first (depth-first):
    ...
    2. For each {entry} in {sub-entries}:
        ...
        5. {declared}: bash: `git config -f {cwd}/.gitmodules submodule.{sub-name}.branch`
        6. If {declared} is empty: Exit process — submodule {sub} has no `branch =` in .gitmodules; recursive push needs that contract. Declare it (`git config -f .gitmodules submodule.{sub-name}.branch <branch>`) or run `/git doctor` to write it. Never inferred from the checked-out branch — that would silently push wherever HEAD happens to point
        7. If {current} is `HEAD` (detached): normalize onto {declared} without discarding work — {head}: bash: `git -C {cwd}/{sub} rev-parse HEAD`; {decl}: bash: `git -C {cwd}/{sub} rev-parse --verify {declared} 2>/dev/null`. If {decl} is empty: bash: `git -C {cwd}/{sub} checkout -b {declared}`. Else if {decl} == {head}: bash: `git -C {cwd}/{sub} checkout {declared}`. Else: Exit process — {declared} ({decl}) diverges from the detached HEAD ({head}); a commit was made off-branch — resolve manually (commit-time normalization belongs in `/git commit`)
        8. Else if {current} ≠ {declared}: Exit process — submodule {sub} on unexpected branch ({current}, expected {declared}); manual handling
        9. Call: verbs/push.md --cwd {cwd}/{sub} --branch {declared} — recursive call handles sub-submodules and the fetch+rebase+ff-push of {sub} itself
...
4. Auto-commit pending changes:
    ...
    2. If {pending} non-empty:
        1. Call: verbs/commit.md --cwd {cwd}
