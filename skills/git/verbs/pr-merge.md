...
- `partials/...` — none; the merge gate is the shared `scripts/pr.py`.
...
- **The gate is `scripts/pr.py`.** Merge never re-computes review/CI/mergeability — it runs `pr.py gate` and acts on the JSON verdict, rather than re-invoking the status verb. The verdict is computed fresh at merge time.
...
2. {status}: bash: `uv run ${CLAUDE_SKILL_DIR}/scripts/pr.py gate --branch {branch}` — the gate; run directly, computed fresh
