...
- **Commit messages via a message file, never inline `-m` with backticks** — backticks in a double-quoted `-m` execute as command substitution and silently mangle the message. (Incident: same day, same PR.) The plan-file commit flow satisfies this by construction.
