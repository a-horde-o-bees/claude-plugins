...
Render a markdown artifact to PDF with the bundled `scripts/pdf-render.py`. Work performed on an existing PDF — reading, merging, splitting, forms, OCR — belongs to the base `pdf` skill; the two are complementary.

Prerequisite: `uv`. The renderer carries PEP 723 inline metadata declaring `reportlab` and `markdown-it-py`, so each `uv run` resolves dependencies ephemerally — there is no venv to maintain.
...
A preset is a Python module at `~/.claude/skills/export-pdf/styles/<name>.py` declaring only the constants it wants to override. Anything it leaves out falls through to reportlab's library defaults, never to another preset's values — a module with a single declared constant is valid.

Overridable constants, all optional and each consumed by the renderer only when set:
...
`make_styles()` is optional. Defined, it returns a six-key dict — `h1`, `h2`, `h3`, `h4`, `body`, `bullet_text` — that the renderer uses verbatim. Undefined, the renderer supplies a minimum-sufficient default: sizes only, 18/14/12/11/10/10 pt across those six keys, in reportlab's `Times-Roman` with no font or color opinion.
...
A single property change is a complete, valid preset:
...
For a curated starting point, copy the shipped preset and edit it:

```
cp ~/.claude/skills/export-pdf/styles/compact.py ~/.claude/skills/export-pdf/styles/<your-name>.py
```

The full reportlab `ParagraphStyle` API is available inside `make_styles()` — `fontSize`, `leading`, `alignment`, `spaceBefore`, `spaceAfter`, `leftIndent`, and the rest. Render with `--style <your-name>`.
