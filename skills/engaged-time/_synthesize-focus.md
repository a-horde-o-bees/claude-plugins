...
3. **Assign topics — the global pass, NOT fanned out.** Once every root is synthesized, review all threads at once and assign each one's topic tags. This is also where the vocabulary is tuned to the real threads:
    - `uv run ${CLAUDE_SKILL_DIR}/exchanges.py thread-list` → every thread (key, root, summary, current topics).
    - Refine the vocabulary if the threads warrant it: `uv run ${CLAUDE_SKILL_DIR}/exchanges.py topics --set '{…}'`.
    - `uv run ${CLAUDE_SKILL_DIR}/exchanges.py thread-assign --set '{"<thread_key>": "<topic>" | ["<topic>", …], …}'` — one tag is the norm; a thread spanning the billable boundary carries several, and the supplied set replaces the thread's tags.
