...
Load-bearing choices and the alternatives they beat — the rationale that keeps settled questions settled. `ARCHITECTURE.md` describes the mechanism as it *is*; this file records *why*, so a later change can't quietly reintroduce a discarded approach.
