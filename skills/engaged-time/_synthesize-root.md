...
2. `{threads}`: Apply [description-authoring](#description-authoring), [concise-prose](#concise-prose) (inlined in this payload) to: reading `{exchanges}` in full as one root's arc and partitioning it into focus-threads — the disciplines own the structure (voice, concision, outcome over narration, no decorative facts), and this step adds only the coalescing rules:
