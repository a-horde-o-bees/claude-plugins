...
Why we build the way we do. Foundational principles governing all artifacts — code, schemas, tools, instructions, templates, conventions.
...
Enforce correctness through structure, not documentation. When the system's structure prevents incorrect usage, documentation becomes confirmation rather than instruction. Prefer enforcement mechanisms over prose rules at every layer.
...
- Rules and conventions are living context, not immutable constraints — flag conflicts and evaluate which should yield; during normal execution, follow without re-litigating
...
- Diff-based dependency installation runs only when manifests change — no unnecessary work, no judgment required
...
- Tools are named for one verb on one property — not generic operations routed by parameter
...
- Incorporate feedback and directives after confirming the user understands the trade-offs
- Only proceed with a concerning direction after the user has explicitly acknowledged the implications
