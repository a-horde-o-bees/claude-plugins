...
Include what a developer needs to modify the system safely. Exclude user-facing content (e.g., installation, configuration, usage) — that belongs in README.md. Exclude agent-facing procedures (e.g., workflow rules the agent follows, tool invocation patterns for automation) — that belongs in CLAUDE.md or SKILL.md.
