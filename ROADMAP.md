...
- **Background CI gate via async agent** — replace inline `gh run watch` block in `/checkpoint` with a spawned background subagent that watches CI and pushes a notification on pass/fail. Removes the 30–90s block-time per checkpoint. [idea log](logs/idea/Background%20CI%20gate%20via%20async%20agent.md)
...
- **Background CI gate via async agent** ⭐ priority:high — replace `/checkpoint`'s inline `gh run watch` block with a background subagent that watches CI and pushes a notification on pass/fail. Removes the largest block-time contributor in the checkpoint workflow. [idea log](logs/idea/Background%20CI%20gate%20via%20async%20agent.md)
...
- **Backfill decision rationale from transcripts** — mine past conversation transcripts for decision rationale that never made it to `logs/decision/`. [idea log](logs/idea/Backfill%20decision%20rationale%20from%20transcripts.md)
