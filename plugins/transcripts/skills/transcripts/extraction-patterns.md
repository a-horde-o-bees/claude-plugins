...
**Storage-time (mechanical, banked).** Backfill over the reference corpus (139,484 events, 19 projects) extracted 364 skill-body injections into **145 unique refs**, collapsing **855 KB** of duplicated payload (`refs.dedup_bytes_saved`). The most-reused body (`/process-flow-notation`, ~11 KB) recurred 16×. Storage-time wins are uncontested and land regardless of the read-side question.
