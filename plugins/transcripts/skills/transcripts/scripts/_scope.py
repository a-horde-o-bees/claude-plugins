...
def sessions(
    ...
    Default row: {project, session, n_exchanges, n_described}.
        ...
        SELECT project_name, parent_session,
               ...
               (SELECT COUNT(*) FROM exchanges x
                WHERE x.parent_session = events_with_gaps.parent_session
                  AND x.description IS NOT NULL) AS n_described
    ...
    sessions_list: list = []
    for proj, sess, n_ex, ft, lt, bts, n_d in rows:
...
def exchanges(
    ...
    Default row: {project, session, exchange, description}.
    ...
    for sess in sessions_seen:
        ...
        description_rows = conn.execute(f"""
            SELECT exchange, description FROM exchanges
            WHERE parent_session = ? AND exchange IN ({placeholders})
              AND description IS NOT NULL
        """, [sess, *exs]).fetchall()
        for ex, description in description_rows:
            descriptions_by_pair[(sess, ex)] = description
    ...
    for proj, sess, ex, ts, te, ug, at, idle, total in metadata:
        ...
        row: dict = {
            ...
            "session": sess,
            "exchange": ex,
            "description": descriptions_by_pair.get((sess, ex)),
        }
        if want_timeframes:
