...
6. Save extraction script to `blueprint/scripts/{entity_id}-{short_name}.js`
...
Fetch pages via JavaScript `fetch()` within existing browser context on same domain. Avoids page navigation — no snapshot tokens consumed. Requires browser already on same domain (one initial `browser_navigate` establishes domain context).
...
Parse fetched HTML into queryable document without affecting live page. Same DOM APIs as live page — `querySelectorAll`, `getAttribute`, `textContent`.
...
**Pairs with:** Same-Origin Fetch (parse fetched HTML) or any source producing HTML strings.

### Dynamic Accumulation

Process items sequentially, measuring accumulated result size against character budget. Stops before exceeding budget and returns resumption point. Self-regulating — adapts to variable data density.
...
- `nextPage` is exact checkpoint — agent passes as `START_PAGE` in next call
...
**Agent responsibility:** Platform/employer domains (linkedin.com, medium.com) pass regex but are not personal websites — agent filters during registration based on context.
...
python3 ${CLAUDE_PLUGIN_ROOT}/run.py skills.research register-batch --json '[
  ...
  ...
]' --source-url "https://directory-url.com" --db blueprint/data/research.db
```
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/run.py skills.research remove notes --entity-id {directory_id} --note-ids PROGRESS_NOTE_ID --db blueprint/data/research.db
python3 ${CLAUDE_PLUGIN_ROOT}/run.py skills.research upsert notes --entity-id {directory_id} --notes "[CRAWL PROGRESS]: Processed pages 1-N of M. X entities registered. Resume on page N+1." --db blueprint/data/research.db
```

## Recipes

Proven combinations of primitives for specific directory types. When directory matches a recipe, follow it. When no recipe fits, compose new one from available primitives and document it.

### Paginated HTML with Data Attributes

**Directory type:** Server-rendered pages with member data embedded in DOM data attributes (e.g., `data-bio`, `data-member-name`). Content appears via JS modals or toggles but data is in HTML.

**Primitives:** Same-Origin Fetch + DOMParser Extraction + Dynamic Accumulation + Domain Regex Filtering + HTML Stripping + Batch Registration + Progress Note Update
...
**Primitives:** (Browser navigation + snapshot) OR (API endpoint discovery + Same-Origin Fetch) + Dynamic Accumulation + Batch Registration + Progress Note Update
...
**Directory type:** Directory with discoverable REST/GraphQL API returning structured data (JSON).

**Primitives:** Same-Origin Fetch (to API endpoint) + Dynamic Accumulation + Batch Registration + Progress Note Update
...
**Directory type:** Curated lists, blog posts, or articles linking to example entities. No pagination or dynamic content — each page is standalone source.

**Primitives:** Same-Origin Fetch (or WebFetch) + DOMParser Extraction + Batch Registration
...
- **Location:** `blueprint/scripts/{entity_id}-{short_name}.js`
- **Pointer:** `[CRAWL SCRIPT]: blueprint/scripts/{entity_id}-{short_name}.js` note on directory entity
...
Examine all crawl scripts across directories: `ls blueprint/scripts/`
...
| `[CRAWL SCRIPT]:` | File path pointer to executable JS script | Stable — set once when script saved |
