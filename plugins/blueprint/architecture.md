...
The research database supports the `/blueprint-research` skill across four phases: scoping, deep research, analysis, and implementation blueprint. It stores entities (tools, projects, sources being studied), their notes (atomic facts), assessment criteria, and analysis outputs. Every write operation enforces business rules (URL dedup, stage validation, criteria constraints) so agent instructions focus on research decisions, not data manipulation.
...
`criteria_notes` uses ON DELETE CASCADE on both foreign keys. All other tables use manual cascade in Python. The junction table deviation is justified: it has no dependents, and automatic cascade ensures criteria links never orphan when notes or criteria are deleted (including during stage downgrade, which bulk-deletes notes).
...
Single-letter prefixes generated by `_next_id`: `e` (entity), `n` (note), `c` (criterion). IDs are stable, never reused, and identical in input and output.
...
- **Hardline**: entity fails any → rejected (stage=rejected, relevance=0)
...
| Mode | Agent work | Phase |
|------|-----------|-------|
| `example` | Deep research as comparable tool | Phase 2 |
| `directory` | Directory crawl for entity listings | Phase 1 |
| `context` | Context research for domain knowledge | Phase 1 |
| `unclassified` | Marker — needs assessment pass | Resolved by assess-entity |

If an entity has two modes, it needs two different kinds of agent work in different phases.
