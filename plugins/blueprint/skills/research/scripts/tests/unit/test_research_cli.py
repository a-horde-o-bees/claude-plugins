...
RESEARCH_CLI = str(SCRIPTS_DIR / "research_cli.py")


def run_cli(*args: str, db: str) -> subprocess.CompletedProcess:
    """Run research CLI with --db flag."""
    cmd = [sys.executable, RESEARCH_CLI, args[0], *args[1:], "--db", db]
...
class TestNormalizeURLCLI:
    def test_normalize_url_command(self) -> None:
        cmd = [sys.executable, RESEARCH_CLI, "normalize-url", "https://www.Example.COM/path/"]
