...
- References to plugin skills use the qualified `/plugin:skill` form in every context the agent authors or emits — PFN `skill:` invocations, emitted strings, documentation, agent output, user-facing messages. Unqualified forms rely on the CLI router's namespacing fallback, which does not apply elsewhere (e.g., Skill tool, emitted strings, agent-produced docs) and can collide across installed plugins. User-typed CLI input is exempt — the router handles it
