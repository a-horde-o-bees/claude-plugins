...
Path resolution always routes through `dependencies.environment` — never through direct environment reads, never from `os.getcwd()`, never through parent walks from arbitrary paths. The module is vendored into every skill that uses it at `<skill>/scripts/environment.py` (canonical at project-root `dependencies/environment.py`) so it imports identically from any location:
...
Entry points take only their own domain-specific arguments. Project and plugin paths are resolved internally via `dependencies.environment` (`get_project_dir()`, `get_plugin_root()`, `get_plugin_data_dir()`) — see *Project, Plugin, and Data Directory Resolution*. Never accept those paths as parameters.
...
Subsystems describe their current state through `extra` entries. Skills prescribe next steps by reading status output and interpreting it — systems do not embed corrective commands.

- `overall status` — single line summarizing infrastructure state (standard label across systems)
...
Cross-package — always-on primitives via the vendored `dependencies/` package, install/init/status orchestration via `systems.setup`:

```python
from dependencies import environment        # path resolution (always-on, no setup required)
from dependencies.errors import NotReadyError
