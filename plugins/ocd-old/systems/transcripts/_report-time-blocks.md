...
Each block carries a **Summary Purpose** — a Purpose Statement-style line capturing the unified objective of the contained exchanges; broader than the per-exchange purposes that compose it.
...
2. Pull `avg_user_time_s`:
    - bash: `ocd-run transcripts settings`
    - {avg-user-s} = `derived.avg_user_time_s.value` from response

3. List sessions in scope:
    - bash: `ocd-run transcripts sessions {format-args} --show timeframes`
...
5. For each {session} in {sessions}:
    1. Pull full exchange detail:
        - bash: `ocd-run transcripts exchanges --session {session} --show messages metrics timeframes`
    2. {purposes} = empty map
    3. For each {exchange} in response:
        1. Read user_msg and assistant messages
        2. Compose a Purpose Statement-style summary — scope + role, no mechanics, no history; one line, typically a noun or verb phrase
        3. Set `{purposes}[{exchange.exchange}] = {purpose}`
    4. Persist purposes for the session:
        - bash: `ocd-run transcripts purposes-set {session} '<json of purposes>'`
...
6. Consolidate exchanges into blocks:
    ...
    4. Compose a Block Summary Purpose using Purpose Statement framing — broader than per-exchange purposes; captures the unified objective of the contained exchanges
    5. Auto-tag each block as in-scope (`yes`) or out-of-scope (`no`) for {project} from the Block Summary Purpose

7. Present proposed blocks for user review:
    1. Render proposal table for user — Exchanges, Summary Purpose, estimated Time (computed per step 8), In scope?
    2. Ask user to approve or adjust — Q# format with lettered options
    3. If user requests adjustments:
        1. Apply changes (re-group blocks, re-classify, re-word summary purposes as directed)
...
9. Render the report:
    ...
    2. For each day in chronological order:
        ...
        2. Day table — columns: `Exchanges`, `Summary Purpose`, `Time`, `In scope?`
...
| Exchanges | Summary Purpose | Time | In scope? |
|---|---|---:|---|
