...
Plugin root on sys.path is what makes `from dependencies import environment`
and `from dependencies.errors import NotReadyError` resolve — the vendored
`dependencies/` package lives at plugin root so propagated primitives are
