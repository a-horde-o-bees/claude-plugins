...
def skills_resolve(name: str) -> Path | None:
    ...
    Accepts bare name (`navigator`), slash-prefixed (`/navigator`),
    plugin-qualified (`ocd:navigator`), or fully qualified
    (`/ocd:navigator`) — a leading slash and any `plugin:` prefix are
