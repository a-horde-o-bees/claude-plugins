...
Per-entity evidence files that back a subtopic's `_CONSOLIDATED.md` synthesis. Samples live under `logs/research/<subject>/<subtopic>-samples/<entity>.md` and mirror the subtopic's `_TEMPLATE.md` heading tree so cross-sample tallying produces comparable counts.

## What Qualifies

A sample is one data point for the research subject — one repo when studying ecosystems, one file when studying content standards, one implementation when studying a design space. The sample captures what *this specific entity* does for each purpose the subtopic investigates.

A sample file is not a narrative about the entity. It's evidence organized to be scanned and tallied alongside siblings.

## Entity Identifier

Each sample file is named for its entity using a filename scheme the subtopic declares. Keep the scheme stable across all samples in the subtopic:
...
The first sample authored in a new subtopic sets the scheme; later samples match.

## Entry Structure

The sample file mirrors the subtopic's `_TEMPLATE.md` heading tree one-to-one. Every heading in `_TEMPLATE.md` appears in every sample file; samples that don't capture a particular purpose mark it explicitly (see *Not captured*) rather than omitting.
...
Summary metadata about the entity that research needs to cite — size, language, activity signals, last commit date, whatever the subtopic's tallying needs.
...
Entity-specific observations that don't fit any purpose section — bugs, unusual choices, ecosystem-visible defects. Consumed by `_CONSOLIDATED.md`'s pitfalls slots.
...
- **Every `_TEMPLATE.md` heading appears as a section.** Missing sections break tallying. Run `ocd-run log research compliance --subject <name>` to verify the corpus matches the template.
- **Heading text matches `_TEMPLATE.md` exactly** so scripts can scan for chain keys.
...
`_CONSOLIDATED.md`'s adoption counts aggregate from the samples. For a count like "14/54 repos declare non-trivial `userConfig`":
...
When `_TEMPLATE.md` adds a new purpose (discovered in a later research wave), samples need the purpose backfilled. The refresh:
...
Never leave samples inconsistent with `_TEMPLATE.md`'s heading set — tallying breaks silently when some samples have a purpose and others don't. The compliance verb catches this.

## Lifecycle

Semi-permanent. Samples accumulate as the subtopic grows. Delete only when the entity is genuinely removed from the research population (a repo archived and no longer relevant, a file removed from the codebase), or when a sibling sample subsumes the entity.
