...
def paths_upsert(
    ...
    absolute_entry = environment.get_project_dir() / entry_path if entry_path else environment.get_project_dir()
    ...
    try:
        ...
        if purpose is not None and entry_type == "file":
            metrics = _compute_file_metrics(environment.get_project_dir() / entry_path)
