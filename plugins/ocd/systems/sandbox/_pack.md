...
> Preconditions — pack must run on a clean main checkout; the default scope's paths must exist; no conflicting branch or sibling may already exist.

5. Verify preconditions:
    1. {current-branch} = bash: `git rev-parse --abbrev-ref HEAD`
    2. If {current-branch} is not `main`: Exit to user: pack requires main checkout; currently on {current-branch}
    3. bash: `git status --short`
    4. If output is non-empty: Exit to user: working tree has changes — commit or stash before packing
    ...
    7. If exit 0: Exit to user: local branch {branch} already exists
    8. bash: `git ls-remote --exit-code --heads origin {branch}`
    9. If exit 0: Exit to user: remote branch {branch} already exists on origin
    10. If {sibling-path} exists on disk: Exit to user: sibling path {sibling-path} already exists
...
7. Scan for external references:
...
9. {status-md-content} = empty accumulator

10. For each {tendril} in {auto-delete}:
    1. Delete the referenced line (or the full table row) from the tendril's file

11. For each {tendril} in {auto-migrate}:
...
12. For each {tendril} in {interactive}:
...
> Verification — confirm the tendril set on main is empty except for explicit `leave` overrides.

13. Re-run the tendril scan from step 7 over the updated tree
14. {remaining} = re-scan output minus any entries the user chose `leave` on
15. If {remaining} is non-empty:
    1. Exit to user:
        - unexpected tendrils still present after auto-handle + interactive
        - {remaining}
        - re-run pack once resolved

> Transition — two commits land: one on main with tendril cleanup + scope removal, one on the sibling branch that carries the `_status.md` if content was migrated. The sibling is branched from main BEFORE the scope-removal commit, so the scope paths are already present in the sibling without needing manual restoration. Main tree stays on main throughout; no checkouts.

16. Execute transition:
    1. bash: `ocd-run sandbox worktree-add {sibling-name} --branch {branch} --base-ref main`
    2. On main tree: remove scope paths
        1. For each {path} in {scope}.paths: bash: `git rm -r {path}`
        2. Stage tendril modifications from steps 10–12: bash: `git add -u`
...
17. Return to caller:
    ...
    - main: scope paths removed; HEAD advanced by 1 commit; pushed
