...
from subsystems.governance import (
    governance_list,
    governance_match,
    governance_order,
)
from subsystems.governance._frontmatter import (
    normalize_patterns,
    parse_governance,
    read_frontmatter,
)
...
class TestGovernanceMatchExcludes:
    ...
    def excludes_files(self, project_dir):
        _write_governance_file(project_dir / ".claude/rules/principles.md", "*")
        _write_governance_file(
            project_dir / ".claude/conventions/server.md", "subsystems/navigator/*.py",
    ...
    def test_excludes_filters_basename(self, excludes_files):
        result = governance_match(["subsystems/navigator/__init__.py"])
        matched = result["matches"].get("subsystems/navigator/__init__.py", [])
    ...
    def test_excludes_allows_normal_files(self, excludes_files):
        result = governance_match(["subsystems/navigator/server.py"])
        matched = result["matches"]["subsystems/navigator/server.py"]
        assert ".claude/conventions/server.md" in matched
        assert ".claude/conventions/ocd/python.md" in matched

    def test_excludes_filters_helpers(self, excludes_files):
        result = governance_match(["subsystems/navigator/_server_helpers.py"])
        matched = result["matches"].get("subsystems/navigator/_server_helpers.py", [])
        assert ".claude/conventions/server.md" not in matched
        assert ".claude/conventions/ocd/python.md" in matched

    def test_excludes_in_conventions_list(self, excludes_files):
        result = governance_match(["subsystems/navigator/__init__.py", "subsystems/navigator/server.py"])
