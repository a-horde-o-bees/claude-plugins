...
from subsystems.navigator import _skills
from subsystems.navigator._skills import (
...
class TestListSkills:
    def test_lists_from_plugin_dir(self, tmp_env: dict[str, Path]) -> None:
        _write_skill(tmp_env["plugin"] / "skills", "conv", "conventions")
        _write_skill(tmp_env["plugin"] / "skills", "nav", "navigator")
        skills = skills_list()
        assert len(skills) == 2
