...
- **Facade re-exports, not subclassing or injection.** Internals are private to the library and re-exported from `__init__.py` by name. Consumers import `subsystems.navigator` and get the public surface. Internal modules can be restructured without breaking consumers.
...
- **Navigator MCP server** (`plugins/ocd/subsystems/navigator/server.py`) — thin FastMCP adapter. Every tool delegates to a facade function, serializes the result, and returns.
...
- **Convention gate hook** — the related `subsystems/governance` library reads from disk directly, not through navigator. Navigator's `scope_analyze` calls `governance_match` to attach governance data to scanned files.
