...
3. Discover governance levels — bash: `CLAUDE_PROJECT_DIR=$(pwd) python3 ${CLAUDE_PLUGIN_ROOT}/run.py subsystems.governance order --json`
...
3. Spawn:
    1. Call: `${CLAUDE_PLUGIN_ROOT}/subsystems/audit-governance/_audit-workflow-A.md` ({level-files} = {levels}[{current-level}])
    2. Return to caller:
