...
Tool design, server architecture, and data conventions for MCP servers exposed as plugin tools via FastMCP. Each server is a single module under `subsystems/<name>/server.py` that imports a domain library from `subsystems/<name>/` and exposes its functions as MCP tools. Library code (facades, internal modules, CLIs) under `subsystems/<name>/` is governed by general Python conventions, not this one.
...
Each MCP server is a single Python module under `subsystems/<name>/server.py`. The domain library it exposes is a separate package under `subsystems/<name>/`.
...
The MCP adapter module (`subsystems/<name>/server.py`):
...
The library (`subsystems/<name>/__init__.py`) owns all business logic — database operations, parsing, file I/O, decision logic. The MCP adapter is a thin presentation layer. The same functions are callable from the library's CLI (`subsystems/<name>/__main__.py`), tests, or any other code path.
...
"<name>": {
  ...
  "args": ["${CLAUDE_PLUGIN_ROOT}/run.py", "subsystems.<name>.server"]
}
```

For this to work:

- `subsystems/<name>/` must be a proper Python package (contain `__init__.py`)
- Server modules use clean imports: `import subsystems.<name> as _<short>`, `from ._server_helpers import _ok, _err`
...
Shared utility functions used by multiple server modules in the same plugin live in `subsystems/navigator/_server_helpers.py` per `python.md`'s `_helpers.py` standard internal module type. Standard helpers:
...
**Bootstrap location.** The servers package bootstraps `CLAUDE_PROJECT_DIR` from `Path.cwd().resolve()` at import time in `subsystems/navigator/_server_helpers.py` when the variable is missing. Every server module imports `_helpers` (for `_ok`/`_err`), so the bootstrap fires at process start.

**Cwd-derived fallback is MCP-subprocess-only.** `subsystems/navigator/_server_helpers.py` is the only place in the codebase permitted to derive the project directory from cwd. The exception is safe because MCP-server cwd is a Claude Code launch-time contract, not a user-influenced working directory.
