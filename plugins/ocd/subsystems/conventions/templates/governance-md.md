...
- **Path with wildcard prefix**: `**/subsystems/*/server.py` matches MCP server modules at any depth in the directory tree. Use `**/` prefix when the target directory may be nested (e.g. under `plugins/`)
