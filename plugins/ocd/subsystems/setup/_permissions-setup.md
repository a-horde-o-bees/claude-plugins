...
1. Status — bash: `ocd subsystems.setup permissions status`
...
5. Else: {scope} = user
6. Install — bash: `ocd subsystems.setup permissions install --scope {scope}`
7. Present install results
8. Analyze — bash: `ocd subsystems.setup permissions analyze`
9. If redundancy count > 0:
    ...
    4. If cleanup chosen:
        1. Clean — bash: `ocd subsystems.setup permissions clean --scope {other-scope}`
