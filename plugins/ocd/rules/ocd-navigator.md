...
| `scan <path>` | Explicit filesystem sync; other commands auto-scan so explicit scan is rarely needed; use when you want to see the scan report |
...
- **Navigator `list`** — enumerate file paths under a directory with exclusion rules applied; for tool consumption (e.g., convention checking, batch operations)
