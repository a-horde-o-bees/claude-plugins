...
Git push is blocked by the skill executor before you were spawned. Any `git push` attempt will fail with `fatal: '/dev/null' does not appear to be a git repository` — this is expected safety behavior, not an error to investigate or work around.

## Steps

1. Read the target skill's SKILL.md at the path provided by the skill executor
...
4. Invoke the skill via the Skill tool with the arguments provided by the skill executor
5. Follow the skill's instructions as loaded by the Skill tool
    ...
    - If the skill attempts git push: the push will fail with the expected safety error — record "push blocked as expected" and continue
