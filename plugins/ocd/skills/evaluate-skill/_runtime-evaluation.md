...
1. Block git push — bash: `git config --worktree remote.origin.pushurl "file:///dev/null"`
    - This command produces no output. That is expected.
    - Any subsequent push attempt will fail with `fatal: '/dev/null' does not appear to be a git repository` — this is expected safety behavior, not an error to investigate or work around.
2. Read the target skill's SKILL.md at the path provided by the orchestrator
3. Note the skill's documented claims:
...
4. If the route requires preconditions (e.g., uncommitted changes for a commit skill, specific file state):
    1. Set up the minimum preconditions needed in the worktree
5. Invoke the skill via the Skill tool with the arguments provided by the orchestrator
6. Follow the skill's instructions as loaded by the Skill tool
...
7. After the skill completes (or exits), compare what happened against the documented claims from step 3:
