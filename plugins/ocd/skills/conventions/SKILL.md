...
8. Enumerate targets — run navigator CLI to get filtered file list
    ```bash
    python3 ${CLAUDE_PLUGIN_ROOT}/run.py skills.navigator list {target-directory} --exclude ".claude/*" [--pattern "..."]
    ```
...
1. Get evaluation order — run conventions CLI `list-self` command
    ```
    python3 ${CLAUDE_PLUGIN_ROOT}/run.py skills.conventions list-self
    ```
