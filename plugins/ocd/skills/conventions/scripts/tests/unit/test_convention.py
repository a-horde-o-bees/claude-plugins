...
def _write_convention(conv_dir, name, pattern, content="# Test"):
...
class TestExtractPattern:
    def test_extracts_pattern_from_frontmatter(self, tmp_env):
        path = _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
        assert _extract_pattern(path) == "*.py"

    def test_returns_none_without_frontmatter(self, tmp_env):
        path = os.path.join(tmp_env["conv_dir"], "no_front.md")
        with open(path, "w") as f:
            f.write("# No Frontmatter\nContent.\n")
        assert _extract_pattern(path) is None

    def test_returns_none_for_missing_pattern_field(self, tmp_env):
        path = os.path.join(tmp_env["conv_dir"], "no_pattern.md")
        with open(path, "w") as f:
            f.write("---\nname: test\n---\n\n# Content\n")
        assert _extract_pattern(path) is None

    def test_handles_single_quoted_pattern(self, tmp_env):
        path = os.path.join(tmp_env["conv_dir"], "single.md")
        with open(path, "w") as f:
            f.write("---\npattern: '*.py'\n---\n\n# Content\n")
        assert _extract_pattern(path) == "*.py"

    def test_handles_unquoted_pattern(self, tmp_env):
...
class TestSyncPatterns:
    def test_syncs_new_files(self, tmp_env):
    ...
    def test_uses_cache_on_second_call(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
    ...
    def test_removes_stale_cache_entries(self, tmp_env):
        path = _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
    ...
    def test_updates_changed_files(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
        sync_patterns(tmp_env["db_path"], tmp_env["conv_dir"])
    ...
    def test_empty_directory(self, tmp_env):
        patterns = sync_patterns(tmp_env["db_path"], tmp_env["conv_dir"])
        assert patterns == {}

    def test_nonexistent_directory(self, tmp_env):
        patterns = sync_patterns(
            tmp_env["db_path"], os.path.join(tmp_env["tmpdir"], "nonexistent")
        )
        assert patterns == {}

    def test_skips_files_without_pattern(self, tmp_env):
        path = os.path.join(tmp_env["conv_dir"], "no_pattern.md")
        with open(path, "w") as f:
            f.write("---\nname: test\n---\n\n# No pattern field\n")
...
class TestMatchConventions:
    def test_matches_by_extension(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
    ...
    def test_matches_by_filename(self, tmp_env):
    ...
    def test_multiple_patterns_match(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
        _write_convention(tmp_env["conv_dir"], "cli.md", "*_cli.*")
    ...
    def test_multiple_input_files(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
    ...
    def test_deduplicates_matches(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "python.md", "*.py")
    ...
    def test_cli_pattern_matches_any_extension(self, tmp_env):
        _write_convention(tmp_env["conv_dir"], "cli.md", "*_cli.*")
...
class TestGetConventionContent:
    def test_strips_frontmatter(self, tmp_env):
    ...
    def test_multiple_conventions(self, tmp_env):
    ...
    def test_handles_missing_file(self, tmp_env):
