...
from skills.governance._db import get_connection, init_db, SCHEMA
from skills.governance._frontmatter import (
    normalize_patterns,
    parse_governance,
    read_frontmatter,
)
...
class TestGovernanceLoad:
    ...
    def test_populates_depends_on(self, gov_env):
        ...
        assert (".claude/conventions/python.md", ".claude/rules/design-principles.md") in edges
        assert len(edges) == 2
        conn.close()
