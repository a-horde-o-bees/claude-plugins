...
Blank context is structurally required — the agent must discover everything from scratch to test whether documentation is sufficient. This is the reason separate agents are spawned rather than processing inline.
...
- Multi-scenario runs agents sequentially — each scenario gets a fresh blank-context agent, one at a time
