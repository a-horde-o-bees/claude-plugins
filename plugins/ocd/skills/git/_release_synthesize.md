...
6. Recommend the next version:
    ...
    4. {bump-axis-rationale}: one or two sentences naming which categories drove the chosen axis (e.g., "y-bump: Added entries for new `/ocd:transcripts report` verb and `/ocd:git release` skill; no Removed or breaking Changed entries triggered x")

7. Compose the CHANGELOG section:
