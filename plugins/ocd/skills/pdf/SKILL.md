...
1. If --set-css:
    1. If {set-css} is `github` or `compact`:
        ...
        3. bash: `cp ${CLAUDE_PLUGIN_ROOT}/skills/pdf/{set-css}.css .claude/ocd/pdf/css/`
...
7. For each --src:
    ...
    5. Convert — bash: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/pdf/generate.py --src {src} --css {stylesheet} --dest {out}`
    6. Report result
