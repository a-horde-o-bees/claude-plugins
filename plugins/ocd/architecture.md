...
Template files in `subsystems/rules/templates/` deploy to `.claude/rules/` via `/ocd:setup init`. Users own the deployed copies — they can inspect, edit, or delete them.
...
| `permissions` | `subsystems/permissions/` | Permissions subsystem — reports auto-approve coverage; specialized CLI ops (`status`, `install`, `analyze`, `clean`) manage recommended patterns across project and user scopes. | [README](subsystems/permissions/README.md) |
...
Consumers within this plugin: the `convention_gate` hook imports `subsystems.governance`; the navigator MCP server imports `subsystems.navigator`; plugin orchestration (`run_init` / `run_status`) discovers and calls every `subsystems/*/_init.py` uniformly for per-subsystem deployment and reporting.
...
| `_orchestration.py` | `run_init` and `run_status` entry points — discover every subsystem under `subsystems/` and dispatch uniformly |
