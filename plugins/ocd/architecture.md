...
Template files in `lib/rules/templates/` deploy to `.claude/rules/` via `/ocd:plugin install`. Users own the deployed copies — they can inspect, edit, or delete them.
...
Rules use the template-deployed model: templates in `lib/rules/templates/` are the source of truth; deployed copies in `.claude/rules/ocd/` are derived (gitignored, regenerated by `/ocd:plugin install`). A pre-commit hook runs `scripts/sync-templates.py` so that dev edits to deployed copies flow back to templates before each commit.
...
| `update-system-docs` | Maintain project documentation by deriving it from code reality (design-only placeholder; see `skills/update-system-docs/_DESIGN.md` for the plan) |
...
├── lib/
│   ├── governance/              — governance library (see lib/governance/architecture.md)
│   └── navigator/               — navigator library (see lib/navigator/architecture.md)
├── servers/
│   ├── navigator.py             — navigator MCP server (thin adapter over lib/navigator)
│   └── _helpers.py              — CLAUDE_PROJECT_DIR bootstrap + response-envelope helpers
├── skills/
