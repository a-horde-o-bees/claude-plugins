...
Template files in `systems/rules/templates/` deploy to `.claude/rules/` via `/ocd:setup init`. Users own the deployed copies — they can inspect, edit, or delete them.
...
Rules use the template-deployed model: templates in `systems/rules/templates/` are the source of truth; deployed copies in `.claude/rules/ocd/` are derived (gitignored, regenerated by `/ocd:setup init`). A pre-commit hook runs `scripts/sync-templates.py` so that dev edits to deployed copies flow back to templates before each commit.
