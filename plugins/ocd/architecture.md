...
Template files in `templates/rules/` deploy to `.claude/rules/` via `/ocd:plugin install`. Users own the deployed copies — they can inspect, edit, or delete them.
...
Rules use the template-deployed model: templates in `templates/rules/` are the source of truth; deployed copies in `.claude/rules/ocd/` are derived (gitignored, regenerated by `/ocd:plugin install`). A pre-commit hook runs `scripts/sync-templates.py` so that dev edits to deployed copies flow back to templates before each commit.
