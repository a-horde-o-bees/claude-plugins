...
2. AskUserQuestion — set `refs/remotes/origin/HEAD` → `{default}` now? It is a local, reversible pointer that makes `git symbolic-ref` resolve without a network call, so every skill's default-branch lookup is correct (not just falling back to `main`).
...
- `origin/HEAD`: set → {default} | left unset (skills resolve via `gh api` / `main` fallback)
