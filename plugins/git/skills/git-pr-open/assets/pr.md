...
> How this project opens and merges pull requests. Read by `/git:git-pr-open`, `/git:git-pr-status`, and `/git:git-pr-merge`. Solo-vs-team gating is detected at runtime from branch protection — this file records preferences, not the live gate.
...
- On a protected base, `/git:git-pr-merge` waits for required approvals + green CI (team path); on an unprotected base it merges immediately once CI is green (solo path)
