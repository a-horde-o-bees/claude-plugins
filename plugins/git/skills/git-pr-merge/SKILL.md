...
- `/git:git-pr-cleanup` — chained by `--cleanup`.
...
4. If {pr-exists} is false: Exit process — no open PR for {branch}; run `/git:git-pr-open` first
...
10. If {cleanup}: skill: `/git:git-pr-cleanup --base {base} --head {branch}`
