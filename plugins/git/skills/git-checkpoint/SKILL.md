...
4. Routing pre-flight + land PR-governed submodules (before the parent commits) — one mechanical pass routes the parent and every submodule; any gap halts up front:
    ...
    3. If {routes}.ok is false: Exit process — routing is ambiguous; resolve every gap, then re-invoke. List each {routes}.gaps entry as `{repo}: {gap} → {fix}`. Run `/git:git-doctor` to write the native keys it can. No mid-flight bootstrap, no guessing — parent and submodule gaps halt the same way
    ...
    5. For each {repo} in {routes}.repos with {repo}.path ≠ `.` (the submodules):
        ...
        4. {repo}.integration is `pr`:
            ...
            5. {sub-report}: skill: `/git:git-checkpoint --path pr` + ` --auto` if {auto} — full lifecycle in {sub}: recurse its own sub-submodules, commit → push → CI → open PR → merge (gated) → cleanup
