...
2. If {skill} is empty: Exit to user: skill name required — usage: `/rules remove <skill> [--scope <user | project>]`
...
5. If {target-file} doesn't exist: Exit to user: `/{skill}` not promoted at {scope} scope — {target-file} doesn't exist
...
8. If block missing: Exit to user: `/{skill}` not promoted at {scope} scope — no sentinel block in {target-file}
9. {block-lines}: lines between sentinels (exclusive)
10. If `Read /{skill}` not in {block-lines}: Exit to user: `/{skill}` not promoted at {scope} scope — no matching line in block
...
12. If {block-lines} is empty after removal:
    ...
    4. Exit to user: demoted `/{skill}` at {scope} scope — block emptied, sentinels removed from {target-file}

13. Else block retains other promotions:
    1. Write: {target-file} ← updated {body}
    2. Exit to user: demoted `/{skill}` at {scope} scope — removed line from block in {target-file}
