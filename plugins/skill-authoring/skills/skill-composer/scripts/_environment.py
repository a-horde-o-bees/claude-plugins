...
`<plugin>/shared/` propagated by `.githooks/pre-commit`. Drift is caught
...
def get_project_dir() -> Path:
    ...
    Defensive guard: refuses any resolution that lands inside Claude home
    (~/.claude/). Plugin caches under ~/.claude/plugins/cache/ are
    themselves git checkouts; without the guard, `git rev-parse` fired
    from a cache subdirectory returns ~/.claude/ and downstream code
    writes to ~/.claude/.claude/... paths. The corrective error names
    the env var the caller should set.
