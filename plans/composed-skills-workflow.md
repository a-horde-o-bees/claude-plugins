...
Workflow for maintaining shareable composed skills inside this monorepo using `progressive-composer` + Vercel's `npx skills` (or `/plugin install`) as the install tooling.

## Why this workflow

progressive-composer's default destinations (`user`, `project`) are right for most users — drop the composition straight into the consumer's Claude Code skills directory. For composed skills the **maintainer** of this repo wants to **share with others**, that default is wrong: the skill is consumed by the maintainer's downstream sessions, not authored as a redistributable artifact.
...
Even though the composition is in a monorepo (not in `<scope>/.claude/skills/`), progressive-composer's drift detection works the same way:
