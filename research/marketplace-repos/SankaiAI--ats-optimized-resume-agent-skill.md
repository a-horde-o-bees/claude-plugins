...
- **Whether the `bin/` directory is actually added to PATH by Claude Code at install time as the bash wrapper's comment claims.** The `resume-skill` comment says "Claude Code automatically adds the plugin's bin/ directory to the Bash tool's PATH when the plugin is installed." This is a claim about Claude Code runtime behavior, not about the repo itself. Verification would require the Claude Code `docs-plugins-reference.md` (present in `research/marketplace-context-resources/`) — I did not cross-reference during this pass. The claim matches the surfaced "bin directory on PATH" pattern from the plugin reference docs, but I did not re-verify against the current doc version.
