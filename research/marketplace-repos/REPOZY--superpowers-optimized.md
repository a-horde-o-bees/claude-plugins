...
- **Sub-event matcher behavior on `resume`** — inferred from hook structure (unscoped SessionStart fires, scoped does not) but not verified by reading Claude Code docs against the observed JSON. The context docs under `research/marketplace-context-resources/docs-hooks.md` could confirm.
