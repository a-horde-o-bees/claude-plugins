# Sandbox Test: /ocd:sandbox lifecycle

End-to-end exercise of the durable sandbox verbs (`new`, `pack`, `open`, `close`, `unpack`, `list`) against a disposable sibling project. Verifies the full lifecycle — sibling worktree creation, edit cycles, rebase on main advancement, and merge back — without touching the parent repo. Spec content still references the pre-refactor substrate in places (checked-out dev branches on the main tree); deeper rewrite to reflect sibling-worktree mechanics is tracked under Phase 8 of the sandbox consolidation.
...
- Invoke `/ocd:sandbox pack boxprobe:target` from `{sandbox}`
- Expect:
...
- Invoke `/ocd:sandbox open boxprobe:target`
- Expect:
    - Checked out `sandbox/boxprobe/target`
...
- Invoke `/ocd:sandbox close`
- Expect:
    - Back on main
    - `sandbox/boxprobe/target` persists with the edit commit
    - Output names next action to resume: `/ocd:sandbox open boxprobe:target`
...
- Invoke `/ocd:sandbox packes` while on dev — expect state `open`
- Close, invoke `/ocd:sandbox packes` — expect state `closed`
...
- Invoke `/ocd:sandbox unpack boxprobe:target` from main
- Expect:
    - `--no-ff` merge of dev into main, producing merge commit `unbox boxprobe:target`
    - Push main to origin
    - Delete `sandbox/boxprobe/target` local + remote
