...
- **Transcripts modernization** — [plan](plans/transcripts.md). **Unblocked 2026-05-21** by skill-architecture Phase 1 — the resolver chain is now captured in `plugins/skill-authoring/skills/skill-architecture/assertions/platform-discovery/project-dir-resolution.md`. Two coupled workstreams: (A) move DB out of project tree to `~/.claude/transcripts/` (security driver — per-project DB pollutes any project lacking the gitignore entry, while auto-sync pulls all sessions across projects); (B) hash-reference table for repeated content (skill bodies, system-reminder lists) to shrink DB and save read-time context. Workstream A ships first; B has an empirical gate at B3. Two open questions still gate A2/A5: T1 (default-scope behavior) and T4 (state-location framework reconciliation). Promotes to In progress when user signals start.
...
- **Phase D — navigator off MCP** — [plan](plans/architecture-refactor.md). CLI-from-MCP migration pattern is the same as transcripts; navigator's state-location move should follow the pattern that `plans/transcripts.md` Workstream A lands (which is itself informed by skill-architecture's project-dir-resolution assertion). Lands at `plugins/navigator/`.
